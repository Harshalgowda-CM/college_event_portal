...
def build_dashboard_data(user):
    ...
    return {
        ...
        "top_events": [{"label": label, "value": value} for label, value in event_counter.most_common(6)],
...
def seed_demo_data():
    ...
    host = cursor.execute("SELECT user_id FROM portal_users WHERE role = 'host' LIMIT 1").fetchone()
    if not host:
        cursor.execute(
            """
            INSERT INTO portal_users (full_name, mobile, password_hash, role, department_name, registration_number)
            VALUES (?, ?, ?, 'host', ?, ?)
            """,

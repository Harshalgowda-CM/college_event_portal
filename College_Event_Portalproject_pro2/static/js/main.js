document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".forgot-toggle").forEach((button) => {
    button.addEventListener("click", () => {
      const targetId = button.dataset.forgotTarget;
      const panel = targetId ? document.getElementById(targetId) : null;
      if (!panel) {
        return;
      }
      panel.classList.toggle("active");
      button.textContent = panel.classList.contains("active") ? "Hide Reset Help" : "Forgot Password?";
    });
  });
